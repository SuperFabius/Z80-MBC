...
S221116 R300417 - HW ref: A041116
...
const byte  autoexecFlagAddr = 12;       // Internal EEPROM address for AUTOEXEC.SUB flag storage
...
byte          autoexecFlag;               // Set to 1 if AUTOEXEC.SUB must be executed at cold boot, 0 otherwise
...
// ------------------------------------------------------------------------------
  ...
  Serial.println("IOS - I/O Subsystem - S221116 R300417");
  Serial.println();
  ...
  // Found Virtual Disk Module. Check Disk Light and AUTOEXEC flags
  {
    ...
    Serial.print("IOS: AUTOEXEC.SUB execution is ");
    if (EEPROM.read(autoexecFlagAddr) > 1) EEPROM.write(autoexecFlagAddr, 0); // Reset AUTOEXEC.SUB flag to OFF if invalid value
    autoexecFlag = EEPROM.read(autoexecFlagAddr);   // Read the previous stored AUTOEXEC.SUB flag
  ...
  // or an invalid bootMode code was read from internal EEPROM
  {
    ...
    if (vDisk == 1)
    // Virtual disk module found, so enable disk light and CP/M related selections
    {
      Serial.println(" 4: CP/M 2.2");
      Serial.print(" 5: Toggle disk light (->");
      if (diskLightFlag) Serial.print("ON");
      else Serial.print("OFF");
      Serial.println(")");
      Serial.print(" 6: Toggle AUTOEXEC.SUB execution (->");
    ...
    if (inChar == '6')
    // Change AUTOEXEC.SUB execution status
    {
      autoexecFlag = !autoexecFlag;               // Toggle AUTOEXEC.SUB executiont status
      ...
      // IO READ operation requested
      {
        ...
        // Execute the requested I/O READ operation
        {
          ...
          case  0x07:
            ...
            //                I/O DATA:    D7 D6 D5 D4 D3 D2 D1 D0
            //                            ---------------------------------------------------------
            //                              0  0  0  0  0  0  0  0    CP/M AUTOEXEC.SUB not enabled
            //                              0  0  0  0  0  0  0  1    CP/M AUTOEXEC.SUB enabled
            //
            // NOTE: Currently only D0 is used
