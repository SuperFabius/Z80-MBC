...
S221116 R120217 - HW ref: A041116
...
// ------------------------------------------------------------------------------
  ...
  Serial.println("IOS - I/O Subsystem - S221116 R120217");
  Serial.println();
  if (moduleGPIO) Serial.println("IOS: Found GPIO Expansion Module"); 

  // Boot selection
  bootMode = EEPROM.read(10);                     // Read the stored boot mode
  if ((bootSelection == 1 ) || (bootMode > 2))
  // Enter in the boot selection menu if USER key was pressed at startup 
  // or an invalid bootMode code was read from EEPROM
  {
    ...
    while ((inChar < minBootChar) || (inChar > '3'));
    Serial.println(inChar);
    bootMode = inChar - '1';                      // Calculate bootMode from inChar
    if (bootMode < 3) EEPROM.write(10, bootMode); // Save to the EEPROM if required
    else bootMode = EEPROM.read(10);              // Reload boot mode if '0' choice selected
  }
