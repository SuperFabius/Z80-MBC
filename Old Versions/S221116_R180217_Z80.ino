...
S221116 R180217 - HW ref: A041116
...
const byte * const bootPh2Table[3] PROGMEM = {bootPh2_A_, bootPh2_B_, bootPh2_C_}; // Phase 2 payload addresses table
...
// ------------------------------------------------------------------------------
  ...
  byte          maxBootMode = 2;            // Maximum allowed value for bootMode
...
// ------------------------------------------------------------------------------
  ...
  // Initialize the EXP_PORT and search for "known" optional modules
  ...
  Wire.beginTransmission(EXT_EEPROM0);            // Set to 1 if Virtual Disk Module is found
  if (Wire.endTransmission() == 0)
  {
    Wire.beginTransmission(EXT_EEPROM1);
    if (Wire.endTransmission() == 0) vDisk = 1;
  }
  ...
  Serial.println("IOS - I/O Subsystem - S221116 R180217");
  Serial.println();
  if (moduleGPIO) Serial.println("IOS: Found GPIO Expansion Module");
  if (vDisk) Serial.println("IOS: Found Virtual Disk Module"); 

  // Boot selection
  if (EEPROM.read(diskLightAddr) > 1) EEPROM.write(diskLightAddr, 0);  // Reset disk light flag to OFF if invalid value
  diskLightFlag = EEPROM.read(diskLightAddr);     // Read the stored disk light flag
  bootMode = EEPROM.read(bootModeAddr);           // Read the stored boot mode
  if (vDisk == 1) maxBootMode = 3;                // Allow CP/M selection if found the virtual disk module
  ...
  // or an invalid bootMode code was read from internal EEPROM
  {
    ...
    // Valid boot mode read, so enable '0' selection
    {
      minBootChar = '0';
    ...
    do
    {
      ...
      // Blink LED_D0 while waiting for the selection
      {
    ...
    if (inChar == '5')
    {
      diskLightFlag = !diskLightFlag;               // Toggle status
      EEPROM.write(diskLightAddr, diskLightFlag);   // Save to the internal EEPROM
    }
    if (inChar == '4') inChar = '3';                // Temporary patch waiting CP/M release...
